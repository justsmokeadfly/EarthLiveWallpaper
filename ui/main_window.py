...
Provides a compact dashboard for live status, wallpaper updates, history,
...
_OUTCOME_COLORS: dict[UpdateOutcome, str] = {
...
_INFO_ROW_KEYS = (
    "last_image_time", "last_update", "next_update", "cache_size", "resolution", "total_updates",
)
...
class MainWindow(ctk.CTk):
    """Top-level EarthLive dashboard window."""
    ...
    def _build_widgets(self) -> None:
        ...
        info_frame = ctk.CTkFrame(
            self, fg_color=theme.COLOR_SURFACE, corner_radius=theme.CORNER_RADIUS
        )
        info_frame.pack(fill="x", padx=theme.PADDING, pady=(0, 10))

        self._info_labels: dict[str, ctk.CTkLabel] = {}
        self._info_titles: dict[str, ctk.CTkLabel] = {}
        for index, key in enumerate(_INFO_ROW_KEYS):
            row = ctk.CTkFrame(info_frame, fg_color="transparent")
            row.grid(row=index // 2, column=index % 2, sticky="ew", padx=10, pady=7)
            info_frame.grid_columnconfigure(index % 2, weight=1)

            title = ctk.CTkLabel(
                row, text=self._tr.get(f"label.{key}"),
                font=(theme.FONT_FAMILY, theme.FONT_SIZE_SMALL),
            ...
            value = ctk.CTkLabel(
                row, text="—",
                font=(theme.FONT_FAMILY, theme.FONT_SIZE_BODY, "bold"),
                text_color=theme.COLOR_TEXT_PRIMARY, anchor="w",
            )
        ...
        self._progress_bar = ctk.CTkProgressBar(
            self, progress_color=theme.COLOR_ACCENT, corner_radius=theme.CORNER_RADIUS,
        )
        ...
        self._update_button = ctk.CTkButton(
            self, text=self._tr.get("button.update_now"), command=self._on_update_now_clicked,
            height=42, font=(theme.FONT_FAMILY, theme.FONT_SIZE_BODY, "bold"),
    ...
    def _make_secondary_button(
        ...
        button = ctk.CTkButton(
            ...
            hover_color=theme.COLOR_SURFACE, corner_radius=theme.CORNER_RADIUS,
        )
        button.grid(row=0, column=column, sticky="ew", padx=3)
        return button
    ...
    def _refresh_status(self) -> None:
        try:
            snapshot: StatusSnapshot = self._controller.get_status_snapshot()
    ...
    def _apply_progress(self, progress: ProgressSnapshot) -> None:
        ...
        stage_key = f"progress.{progress.stage.value}"
        if progress.stage == ProgressStage.DOWNLOADING:
            label_text = self._tr.get(stage_key, current=str(progress.current), total=str(progress.total))
        else:
            label_text = self._tr.get(stage_key)
        self._progress_label.configure(text=label_text)
    ...
    def _apply_snapshot(self, snapshot: StatusSnapshot) -> None:
        if snapshot.last_outcome is not None:
            color = _OUTCOME_COLORS.get(snapshot.last_outcome, theme.COLOR_TEXT_SECONDARY)
            label_text = self._tr.outcome_label(snapshot.last_outcome.value)
        else:
            color = theme.COLOR_TEXT_SECONDARY
            label_text = self._tr.get("status_waiting")
        self._status_dot.configure(text=f"●  {label_text}", text_color=color)
    ...
    def _on_settings_clicked(self) -> None:
        dialog = SettingsDialog(self, self._controller, self._tr, on_language_changed=self._refresh_ui_language)
        dialog.grab_set()
    ...
    def _pause_button_text(self) -> str:
        key = "button.resume" if self._controller.is_paused() else "button.pause"
        return self._tr.get(key)
    ...
    def _refresh_ui_language(self) -> None:
        ...
        for key in _INFO_ROW_KEYS:
            self._info_titles[key].configure(text=self._tr.get(f"label.{key}"))
        self._refresh_status()
