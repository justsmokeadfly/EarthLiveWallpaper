...
**EarthLive Wallpaper v1.4.0** автоматически получает самый свежий доступный снимок Himawari, проверяет скачанные тайлы, собирает их в полноценное изображение диска Земли и устанавливает его как обои Windows.

Программа работает в фоновом режиме и может находиться в системном трее, практически не мешая работе пользователя.

## ✨ Что улучшено в v1.4.0

- 🧹 **Ruff теперь проверяет весь репозиторий**, а не только выборочные изменённые файлы.
- 🔎 Добавлены **security-правила Ruff (`S`)** для поиска распространённых небезопасных паттернов Python-кода.
- 🧠 **mypy теперь проверяет весь репозиторий**, а не только выборочные файлы.
- 🧪 CI продолжает проверять Ruff, mypy и pytest на Windows.
- 📌 Версия метаданных проекта синхронизирована с **1.4.0**.
...
Сборки и релизы создаются автоматически через GitHub Actions из семантических тегов, например `v1.4.0`. Каждая Windows-сборка проходит полную проверку Ruff, mypy, pytest, PyInstaller и установщика.
