...
class SchedulerService:
    """Runs recurring updates in a background thread and supports on-demand triggers."""
    ...
    def start(self, check_interval_hours: float, run_immediately: bool = False) -> None:
        ...
        self._thread = threading.Thread(target=self._loop, name="EarthLiveScheduler", daemon=True)
        self._thread.start()
        _logger.info("Scheduler started (interval=%.1fh, immediate=%s).", check_interval_hours, run_immediately)
        if run_immediately:
    ...
    def trigger_now(self, force: bool = True) -> None:
        """Queue an immediate update; a forced request wins while pending."""
        with self._lock:
    ...
    def _update_next_run(self) -> None:
        ...
        with self._lock:
            self._next_run_at = (
                next_run.replace(tzinfo=UTC) if next_run is not None and next_run.tzinfo is None else next_run
            )
